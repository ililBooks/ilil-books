...
public class KakaoOAuth2Controller {
    ...
    /* Kakao api 로그인 uri 반환 메서드 */
    @GetMapping
    public ResponseEntity<URI> login() {
        return ResponseEntity.ok(kakaoOAuth2UserService.login());
    }

    /* 사용자 정보로 회원 정보 찾기 또는 없을 경우 정보 local DB 저장 메서드 */
    @PostMapping
    public ResponseEntity<KakaoOAuth2UserResponse> getUserInfo(@RequestBody KakaoOAuth2UserRequest kakaoOAuth2UserRequest) {
